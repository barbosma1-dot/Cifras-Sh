...
export default async (req: Request, context: Context) => {
  ...
  try {
    ...
    console.log(`[Netlify Function] Fetching: ${url}`);
    const scraperRes = await axios.get(url, {
      headers: {
    ...
    const status = scraperRes.status;
    ...
    if (status !== 200 || !content || content.length < 100) {
      console.log(`[Netlify Function] Scraper fallback to Gemini Search for: ${url} (Status: ${status})`);
      ...
      if (apiKey) {
        try {
          ...
          console.error("[Netlify Function] Gemini fallback failed:", aiErr.message);

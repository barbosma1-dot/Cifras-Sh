            ...
            if (hasReliableTextLayer(officeWords)) {
              ...
              if (sections.length > 0) {
                ...
                extracted = sections.map(s => {
                  ...
                  // Hino com opção de melodia alternativa leva o número da
                  // opção DEPOIS do incipit: "Hino — {incipit} — Opção N".
                  // s.titleSuffix vem de segmentLiturgyOfHoursPage já
                  // separado do título justamente pra permitir essa ordem
                  // (ver comentário em LiturgySection.titleSuffix).
                  const titleMiddleParts = [incipit, s.titleSuffix].filter(Boolean);
                  const title = isProper
                    ? `${s.title} — ${currentOfficeDayTitle}`
                    : (titleMiddleParts.length > 0
                        ? `${s.title} — ${titleMiddleParts.join(' — ')}`
                        : s.title);
                  return {
                    title,
                    artist: '',

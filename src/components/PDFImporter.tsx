...
0c. PROIBIDO RESUMIR OU ABREVIAR (ERRO GRAVE): Salmos e cânticos litúrgicos costumam ser LONGOS (10, 20, 40+ versículos). Transcreva TODOS os versículos, do primeiro ao último impresso na página, na ordem em que aparecem — mesmo que vários versículos comecem com a mesma fórmula repetida (ex.: cada linha do cântico "bendizei o Senhor!" no Cântico de Dn 3, ou cada "R." de um responsório longo). Repetição de fórmula NÃO é motivo para pular, resumir, ou substituir versículos por reticências/"(...)"/"(continua)". Se o salmo/cântico ultrapassar o fim de uma página e continuar na imagem seguinte do mesmo lote, ainda assim transcreva linha por linha — não corte no meio nem finalize a música cedo demais. Antes de responder, para cada salmo/cântico extraído, confira: "o último versículo do meu 'content' é realmente o último versículo impresso na página, ou eu parei antes do fim?".
...
10b. ANTÍFONA REPETIDA (SÓ NESTE MODO): No Ofício Divino/Liturgia das Horas é normal a MESMA antífona aparecer impressa duas vezes em torno do salmo/cântico que ela abre — uma vez ANTES (abrindo, ex.: "Ant. 1 [texto]") e outra vez DEPOIS (fechando, geralmente só com "Ant. [mesmo texto]" ou "Ant. 1 [mesmo texto]", às vezes com uma variante "T.P.:" — Tempo Pascal — impressa logo abaixo só para o período pascal). Isso NÃO são duas antífonas diferentes: extraia UM ÚNICO objeto "Leitura" para essa antífona (use a ocorrência de ABERTURA, mais completa, como base; se houver uma linha "T.P.:" junto, inclua-a no mesmo content como texto alternativo do Tempo Pascal, não como objeto separado). NUNCA gere dois objetos quase idênticos (um para a abertura, outro para o fechamento) para a mesma antífona.` : ''}
